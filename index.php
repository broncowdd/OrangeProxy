...
/* ADDED BY BRONCO : www.warriordudimanche.net */
if (!empty($_GET['url'])){
	$urlbookmarklet='';
	for ($i=0;$i<strlen($_GET['url']);$i=$i+2){	$urlbookmarklet.=substr($_GET['url'],$i,1);}
	$urlbookmarklet=htmlspecialchars($urlbookmarklet);
}else{
	$urlbookmarklet=''; 
}
...
function afficher_page_form($page) {
	global $urlbookmarklet,$bookmarklet;
	...
	if (!file_exists('style.css')){
		...
		.windows-popup { background-color: #FF5508; border-top: 1px solid #44352C; border-bottom: 1px solid #44352C; clear: both; padding: 30px 0; text-align: center; margin-top: 152px; }
		.windows-popup { background-color: #FF5508; }
	...
	echo '<body>'."\n";
		...
		echo '	<form method="post" action="'.$_SERVER['PHP_SELF'].'" style="text-align:center" name="form">'."\n";
		echo '		<a class="home" href="'.$_SERVER['PHP_SELF'].'">Home</a> / <a class="url" href="'.$url.'">Original link</a> / '.$bookmarklet.'<br/>'."\n";
		if (!empty($urlbookmarklet)){$url=$urlbookmarklet;}
		echo '		<input id="____q" type="text" size="80" name="' . $GLOBALS['q'] . '" value="'.$url.'" />'."\n";
		echo '		<input type="submit" name="go" style="font-size: 12px;" value="Proxyfie"/>'."\n";
		echo '		<br/><hr/>'."\n";
